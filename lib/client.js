  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      var formsNamespace = function (forms) {
        ...
        // An EMPTY describe view means the settings document has not arrived yet
        // (measured on 0.1.7-rc.1: `served=[]` while the document loads), NOT that
        // the host serves nothing. Guessing a candidate name here binds the card
        // to a form that never answers, and every write silently stayed in this
        // browser's mirror instead of the settings document; the caller waits for
        // the view instead.
      ...
      var pickSettingsScope = function () {
        ...
        if (forms && typeof forms.get === "function") {
          try {
            ...
            // null = the describe view has not arrived: no scope yet, the caller
            // waits for it instead of binding to a name the host does not serve.
      ...
      // Host layer first: bind it, and on 0.1.7 WAIT for the settings document —
      // its `configForms` describe view is empty until the document arrives, and
      // binding a guessed namespace there left the card reading and writing this
      // browser's mirror while the settings document sat untouched.
      var seatsOffered = false;
      var offerSeats = function () {
        if (seatsOffered) return;
        seatsOffered = true;
      ...
      var tryAttachHost = function () {
        var scope = pickSettingsScope();
        if (!scope) return false;
        try {
          prefsStore.attachHost(scope);
        } catch (e) {
      ...
      if (!tryAttachHost()) {
        var formsMirror = null;
        try {
          var formsService = getService("configForms");
          formsMirror = formsService && typeof formsService.describe === "function" ? formsService.describe() : null;
        } catch (e) { formsMirror = null; }
        if (formsMirror && typeof formsMirror.subscribe === "function") {
          var offMirror = null;
          var adopt = function () {
            if (tryAttachHost() && typeof offMirror === "function") offMirror();
          };
          offMirror = formsMirror.subscribe(adopt);
          if (typeof formsMirror.ensure === "function") formsMirror.ensure();
          adopt();
        }
        // A host that never serves a document still gets the card, reading and
        // writing this browser's mirror — the documented fallback layer. The
        // delay only exists so a slow document is preferred over the fallback.
        if (!seatsOffered) {
          setTimeout(function () {
            if (!seatsOffered) offerSeats();
          }, 4000);
        }
      }
      ...
      // Registering into a slot a given host never declares is inert, so both are
      // offered together, once the host layer bound (or the fallback timer fired).
    }
