window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var HOST_FIELDS = { dock: 1, lang: 1, levels: 1, fuzzy: 1, hover: 1, debug: 1 };
    ...
    var prefsStore = (function () {
      ...
      var compute = function () {
        ...
        return {
          ...
          levels: over.levels, fuzzy: over.fuzzy, hover: over.hover,
    ...
    function OutlinePanelBody(props) {
      ...
      var lastSeen = react.useRef({
        dock: prefs.dock, y: prefs.y, w: prefs.w || PANEL_WIDTH, h: prefs.h,
        levels: prefs.levels.join(","), fuzzy: prefs.fuzzy
      });
      react.useEffect(function () {
        var seen = lastSeen.current;
        if (prefs.dock !== seen.dock) { seen.dock = prefs.dock; setDock(prefs.dock); }
        if (prefs.y !== seen.y) { seen.y = prefs.y; setPanelY(prefs.y); }
        var nextW = prefs.w || PANEL_WIDTH;
        if (nextW !== seen.w) { seen.w = nextW; setPanelW(nextW); }
        if (prefs.h !== seen.h) { seen.h = prefs.h; setPanelH(prefs.h); }
        var nextLevels = prefs.levels.join(",");
        if (nextLevels !== seen.levels) { seen.levels = nextLevels; setLevels(prefs.levels.slice()); }
        if (prefs.fuzzy !== seen.fuzzy) { seen.fuzzy = prefs.fuzzy; setFuzzy(prefs.fuzzy); }
      }, [prefs]);
      ...
      // The drags only move the live state; the preference effect above persists the
      // settled value (debounced), so there is exactly one writer for these keys.
      var onResizeWDown = function (e) {
        ...
        var move = function (ev) {
          setPanelW(Math.max(180, Math.min(560, startW + (ev.clientX - startX))));
        };
        var up = function () {
      ...
      var onResizeHDown = function (e) {
        ...
        var move = function (ev) {
          setPanelH(Math.max(160, Math.min(window.innerHeight - 60, startH + (ev.clientY - startY))));
        };
        var up = function () {
          window.removeEventListener("pointermove", move);
          window.removeEventListener("pointerup", up);
      ...
      // The glide is drawn frame by frame (rAF, ease-in-out) instead of using the
      // browser's smooth scrolling: some setups carry that call out as an instant
      // jump, and the outline is expected to glide either way. Every frame
      // re-aims at the target's LIVE offset, so the host re-paging the window
      // mid-flight cannot leave the jump short; a short re-check after the
      // landing keeps it honest while the host finishes measuring. A newer jump
      // retires the previous one, and the auto-follow stays out of the way while
      // one is in flight.
      ...
      var glideTo = function (el) {
        ...
        var mine = { done: false };
        glide = mine;
        var frames = Math.round(Math.min(46, Math.max(22, 18 + Math.sqrt(dist) * 0.3)));
        var stepN = 0;
        var step = function () {
          if (mine.done) { if (glide === mine) glide = null; return; }
          stepN++;
          var k = Math.min(1, stepN / frames);
          var eased = k < 0.5 ? 4 * k * k * k : 1 - Math.pow(-2 * k + 2, 3) / 2; // ease-in-out
          var live = scrollTargetOf(el, sp); // the target's offset RIGHT NOW
          sp.scrollTop = from + (live - from) * eased;
    ...
    function TocSettingsCard() {
      ...
      var seg = function (options, current, onPick, inline) {
        return react_jsx_runtime.jsx("div", {
          ...
          children: options.map(function (pair) {
            return react_jsx_runtime.jsx("button", {
              ...
              className: "dqt-psegBtn" + (current === pair[0] ? " dqt-psegOn" : ""),
              children: pair[1]
            }, pair[0]);
          })
        });
      };
      var check = function (on, onToggle) {
        return react_jsx_runtime.jsx("input", {
          type: "checkbox",
          checked: on,
          disabled: !writable,
          onChange: function (e) { onToggle(e.target.checked); },
          className: "dqt-pcheck"
        });
      };
      ...
      var fields = [
        field("lang", T("settings.language"), T("settings.language.tip"),
          seg([["auto", T("settings.lang.auto")], ["zh", T("settings.lang.zh")], ["en", T("settings.lang.en")]], prefs.lang,
            function (v) { commit({ lang: v }); }, true), true),
        field("dock", T("settings.dock"), "",
          seg([["left", T("settings.dock.left")], ["right", T("settings.dock.right")]], prefs.dock,
            function (v) { commit({ dock: v }); }, true), true),
        field("levels", T("settings.levels"), "", levelsControl(true), true),
        field("fuzzy", T("settings.fuzzy"), "", check(prefs.fuzzy, function (v) { commit({ fuzzy: v }); }), true),
        field("hover", T("settings.hover"), "", check(prefs.hover, function (v) { commit({ hover: v }); }), true),
        field("debug", T("settings.debug"), T("settings.debug.tip"), check(prefs.debug, function (v) { commit({ debug: v }); }), true, true)
      ];
    ...
    (function () {
      ...
      s.textContent = "@keyframes dqt-spin{to{transform:rotate(360deg)}}@keyframes dqt-fade-in{from{opacity:0}to{opacity:1}}@keyframes dqt-fade-out{from{opacity:1}to{opacity:0}}" +
        ...
        ".dqt-pcheck{width:16px;height:16px;flex:none;cursor:pointer;accent-color:var(--dsw-alias-brand-primary)}" +
        ".dqt-pcheck:disabled{opacity:.4;cursor:default}";
