window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function OutlinePanel(props) {
      ...
      // per-group collapse map (click a group header to fold its headings)
      var _s11 = react.useState({});
      var collapsedMap = _s11[0];
      var setCollapsedMap = _s11[1];
      var toggleCollapse = function (gi) {
        setCollapsedMap(function (m) {
          var n = {};
          for (var k in m) if (m[k]) n[k] = m[k];
          if (n[gi]) delete n[gi]; else n[gi] = true;
          return n;
        });
      };
      var isCollapsed = function (gi) { return !!collapsedMap[gi]; };
      ...
      // when every outline group is already loaded, it also clicks the
      ...
      var groups = react.useMemo(function () {
        ...
        // also remember each turn's user message key (for header jump)
        for (var i = 0; i < order.length; i++) {
          ...
          if (n0.kind === "user" && turnUserKey[tid0] === undefined) turnUserKey[tid0] = k0;
        ...
        children: [
          ...
          react_jsx_runtime.jsx("div", {
            ...
            children: react_jsx_runtime.jsx("div", {
              style: { direction: "ltr" },
              children: renderGroups(shownGroups, shownTrees, jump, C, Math.max(0, groups.length - visibleCount), activeGroup, isCollapsed, toggleCollapse)
            })
          }),
    ...
    // header click folds the group; clicking the time jumps to the turn start
    function renderGroups(groups, trees, jump, C, offset, activeIdx, isCollapsed, toggleCollapse) {
      var base = offset || 0;
      var activeSet = {};
      ...
      for (var i = 0; i < groups.length; i++) {
        var g = groups[i];
        var gi = base + i;
        var collapsed = isCollapsed ? isCollapsed(gi) : false;
        var items = [];
        items.push(react_jsx_runtime.jsx("div", {
          style: { borderTop: "1px solid " + C.panelBorder, margin: "7px 2px 3px", height: 0 }
        }, "g-sep-" + gi));
        items.push(react_jsx_runtime.jsx("div", {
          onClick: function () { if (toggleCollapse) toggleCollapse(gi); },
          title: collapsed ? "展开该组" : "折叠该组（点击时间可跳转）",
          style: {
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
          ...
          children: [
            react_jsx_runtime.jsx("span", {
              onClick: function (e) {
                e.stopPropagation();
                if (g.userKey) jump(g.userKey);
              },
              title: g.userKey ? "跳转到该回合开头" : "",
              style: { fontSize: 11, color: C.muted, fontWeight: 600, cursor: g.userKey ? "pointer" : "default", padding: "0 3px", borderRadius: 3 },
              children: g.time || " "
            }),
            react_jsx_runtime.jsx("span", {
              style: { fontSize: 9, color: C.muted, opacity: 0.8, flex: "none" },
              children: collapsed ? "▸" : "▾"
            })
          ]
        }, "g-h-" + gi));
        if (!collapsed) items.push(renderNodes(trees[i], 0, jump, C, "g" + gi));
