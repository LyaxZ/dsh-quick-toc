...
A quick conversation TOC plugin for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (DSH): extracts Markdown headings (H1–H6) from AI replies into a navigable outline panel.
...
With the DSH CLI (replace `<repo-path>` with this repo):
...
Restart DSH (`restart-dsh.bat` on Windows or restart the server) and open the Web UI. The outline is collapsed by default — click the small edge handle on the left side of the conversation to expand it.
