...
### Fixed
- **On the 0.1.7 line the settings card's writes only reached the browser**: before the host's settings document had arrived (on a profile whose entry id differs from the candidate names) the plugin bound a form that does not exist — the card opened and took clicks, but every write fell back to the browser mirror with the settings document untouched and no console error. It now waits for the document, and a host that never serves one still gets the card after 4 seconds (the browser mirror, its documented fallback layer).

## [0.7.5] - 2026-09-24

### Changed
- **DSH 0.1.7-rc.1 is supported**: it is the 0.1.7 interface set moved onto the rc channel and shares its settings service and configuration-card slots with 0.1.7-alpha.x, so no code change was needed; it was exercised point by point in an isolated environment (host boot, panel, curtain, Esc, dock switch and collapse).
...
- **The loader row id now equals the settings namespace** (`quick-toc` to `dsh-quick-toc`): from DSH 0.1.7 on, settings are addressed by profile entry id, and an id that does not match the namespace the plugin registered leaves an existing installation's saved preferences unreadable in the import file. With matching names this package can also live in the profile's `dsh.profile.bundles` (0.1.7's plugin manager only offers a configuration page for packages listed there), but **never together with a profile-patch `insert:` of the same name** (that makes the host refuse to boot).
